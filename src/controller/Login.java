...
public class Login extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Matcher matcher = pattern.matcher(nomeUtente);
        boolean flag = false;
        if(!matcher.matches()){
            request.setAttribute("login","errore nel formato del nome.");
            flag = true;
        }
        pattern = Pattern.compile("^[A-Za-z0-9_.-]{5,20}$");
        matcher = pattern.matcher(password);
        if(!matcher.matches()){
            request.setAttribute("login","errore nel formato della password");
            flag = true;
        }
